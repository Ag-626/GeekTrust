...
public class TrainCompositionService {
  ...
  public static List<TrainComposition> filterBogies(String processingStationCode, StationRegistry stationRegistry, List<TrainComposition> trainCompositions){
    ...
    for(TrainComposition comp: trainCompositions){
      ...
      Integer distanceToCompareWith = requiredDistance(processingStationCode, stationRegistry, trainName);
  ...
  private static Integer requiredDistance(String stationCode, StationRegistry stationRegistry, String trainName) {
    Integer distanceToCompareWith;
    try{
      distanceToCompareWith = findDistance(stationCode, stationRegistry, trainName);
    } catch (Exception e) {
      throw new RuntimeException(
          "Unable to find the distance for " + stationCode + "Reason " + e);
    }
    if(distanceToCompareWith == null)
      throw new RuntimeException("The " + stationCode + " this station doesnot exists in the " + trainName + " route");
    return distanceToCompareWith;
  }
}

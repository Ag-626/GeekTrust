...
public class TrainDataLoader {
  private static final int SEGMENT_PARTS = 2;

  public static List<Train> loadTrains(String propertiesFilePath, StationRegistry stationRegistry) {
    List<Train> trains = new ArrayList<>();
    Properties props = new Properties();

    // Load trains.properties from classpath instead of filesystem
    try (InputStream is = TrainDataLoader.class
        .getClassLoader()
        .getResourceAsStream(propertiesFilePath)) {

      if (is == null) {
        throw new RuntimeException(
            "Failed to find train data file in classpath: " + propertiesFilePath);
      }
      props.load(is);
    } catch (IOException e) {
    ...
    for (String trainName : props.stringPropertyNames()) {
      ...
      if (routeDefinition == null || routeDefinition.trim().isEmpty()) {
        continue;
      }
